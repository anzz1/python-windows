...
error(int rc, wchar_t * format, ... )
{
    ...
    va_start(va, format);
    len = _vsnwprintf(message, MSGSIZE-1, format, va);
    va_end(va);

    if (rc == 0) {  /* a Windows error */
        winerror(GetLastError(), win_message, MSGSIZE);
        if (len >= 0) {
            _snwprintf(&message[len], MSGSIZE - len - 1, L": %ls",
                         win_message);
        }
    }
                        ...
                        int display_name_only)
{
    ...
    LONG status = RegOpenKeyExW(root, subkey, 0, flags, &core_root);
    wchar_t message[MSGSIZE];
    DWORD i;
    size_t n;
    BOOL ok, append_name;
    ...
    if (status != ERROR_SUCCESS)
        ...
        for (i = 0; num_installed_pythons < MAX_INSTALLED_PYTHONS; i++) {
            ...
            if (status != ERROR_SUCCESS) {
                ...
                wcsncpy(ip->version, ip_version, MAX_VERSION_SIZE-1);
                ...
                _snwprintf(ip_path, IP_SIZE-1, L"%ls\\%ls\\InstallPath", subkey, ip_version);
                ...
                for (checkp = location_checks; *checkp; ++checkp) {
                    check = *checkp;
                    if (append_name) {
                        _snwprintf(&ip->executable[data_size],
                                     MAX_PATH - data_size,
...
invalid binary type: %X\n",
                            ...
                            if (display_name_only) {
                                ...
                                if (name) {
                                    wcsncpy(ip->exe_display, name+1, MAX_PATH-1);
                                }
                            }
                            if (wcschr(ip->executable, L' ') != NULL) {
                                 ...
                                 * the display name */
                                if (!ip->exe_display[0]) {
                                    wcsncpy(ip->exe_display, ip->executable, MAX_PATH-1);
                                }
...
compare_pythons(const void * p1, const void * p2)
{
    ...
    int result = CompareStringW(LOCALE_INVARIANT, 8, ip2->version, -1, ip1->version, -1); // SORT_DIGITSASNUMBERS = 8
    switch (result) {
    case 0:
...
locate_venv_python()
{
    ...
    _snwprintf(venv_python, MAX_PATH-1, L"%ls\\Scripts\\%ls", virtual_env, PYTHON_EXECUTABLE);
    ...
    ip = &installed_pythons[num_installed_pythons++];
    wcsncpy(ip->executable, venv_python, MAX_PATH-1);
    ip->bits = 0;
    wcsncpy(ip->version, L"venv", MAX_VERSION_SIZE-1);
}
...
locate_all_pythons()
{
    ...
    if (IsWow64Process(GetCurrentProcess(), &f64) && f64) { // TODO: replace import with getprocaddress
 ...
 */
    ...
    _snwprintf(configured_value, MSGSIZE-1, L"py_%ls", key);
...
static wchar_t default_version[] = L"3";
static INSTALLED_PYTHON *
locate_python(wchar_t * wanted_ver, BOOL from_shebang)
{
    static wchar_t config_key [] = { L"pythonX" };
    static wchar_t x32_key [] = { L"X-32" };
    static wchar_t x64_key [] = { L"X-64" };
    ...
    else if (!wcscmp(wanted_ver, L"32")) {
        *x32_key = *default_version;
        wanted_ver = x32_key;
    }
    else if (!wcscmp(wanted_ver, L"64")) {
        *x64_key = *default_version;
        wanted_ver = x64_key;
    }
    if (*wanted_ver) {
...
locate_wrapped_script()
{
    ...
    wcsncpy(p, SCRIPT_SUFFIX, MAX_PATH - (p - wrapped_script_path));
    ...
    if (no_suffix && no_cmdline)
        ...
        if (child_command == NULL)
            error(RC_CREATE_PROCESS, L"unable to allocate %zd bytes for child command.",
                  child_command_size);
        if (no_suffix)
            _snwprintf(child_command, child_command_size - 1, L"%ls %ls", executable, cmdline);
        else
            _snwprintf(child_command, child_command_size - 1, L"%ls %ls %ls", executable, suffix, cmdline);
...
{
    wchar_t * pathext = NULL;
    wchar_t * context = NULL;
    wchar_t * extension;
    COMMAND * result = NULL;
    DWORD     len;

    wcsncpy(path_command.key, name, MAX_PATH-1);
    if (wcschr(name, L'.') != NULL) {
        ...
        pathext = get_env(L"PATHEXT");
        if (pathext) {
            extension = wcstok(pathext, L";");
            while (extension) {
...
{
    wcsncpy(cp->key, name, MAX_PATH-1);
    wcsncpy(cp->value, cmdline, MSGSIZE-1);
}
...
get_version_info(wchar_t * version_text, size_t size)
{
    ...
    if (!version_high && !version_low)
        wcsncpy(version_text, L"0.1", size-1);   /* fallback */
    else {
        maj = HIWORD(version_high);
        min = LOWORD(version_high);
        rel = HIWORD(version_low);
        bld = LOWORD(version_low);
        _snwprintf(version_text, size-1, L"%d.%d.%d.%d", maj, min, rel, bld);
    }
}
    ...
    else
    {
        for (i = 0; i < num_installed_pythons; i++, ip++) {
            ...
            if (wcscmp(ip->version, L"venv") == 0) {
                wcsncpy(version, L"(venv)", BUFSIZ-1);
            }
            else {
                _snwprintf(version, BUFSIZ-1, ver_fmt, ip->version, ip->bits);
            }
...
process(int argc, wchar_t ** argv)
{
    ...
    if (hr != S_OK) {
        ...
        wcsncat(appdata_ini_path, L"\\py.ini", MAX_PATH-1);
    ...
    p[0] = L'\0';
    wcsncat(venv_cfg_path, L"\\pyvenv.cfg", newlen-1);
...
#else
    /* Allocate some extra space for new filenames */
    if (!wcsncpy(launcher_ini_path, argv0, MAX_PATH-1)) {
        error(RC_NO_MEMORY, L"Failed to copy module name");
    }
    ...
    if (p == NULL) {
        ...
        if (!_wcsnicmp(launcher_ini_path, L"python2",7))
            *default_version = L'2';
        //else if (!_wcsnicmp(launcher_ini_path, L"python3",7))
            //*default_version = L'3';

        launcher_ini_path[0] = L'\0';
    }
    else {
        if (!_wcsnicmp(p+1, L"python2",7))
            *default_version = L'2';
        //else if (!_wcsnicmp(p+1, L"python3",7))
            //*default_version = L'3';
        p[0] = L'\0';
        wcsncat(launcher_ini_path, L"\\py.ini", MAX_PATH-1);
    ...
    if (!newcommand) {
        ...
        wcsncpy(newcommand, wrapped_script_path, newlen-1);
        wcsncat(newcommand, L" ", newlen-1);
        wcsncat(newcommand, command, newlen-1);
...
#elif defined(VENV_REDIRECT)
    {
        ...
        if (wcsncat(executable, PYTHON_EXECUTABLE, cch-1)) {
    ...
    if (argc <= 1) {
        ...
        if ((argc == 2) && // list version args
            (!wcsncmp(p, L"-0", /* wcslen(L"-0") */ 2) ||
            !wcsncmp(p, L"--list", /* wcslen(L"--list") */ 6)))
        {
...
installed, use -0 for available pythons", &p[1]);
    ...
    if (!valid) {
            ...
            !_wcsicmp(p, L"-0p") || !_wcsicmp(p, L"--list-paths")))
        {
            ...
            ip = locate_python(default_version, FALSE);
            if (ip == NULL)
